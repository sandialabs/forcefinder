...
def test_reconstructed_response(unit_test_frf, unit_test_force, unit_test_response):
    ...
    training_coordinate = sdpy.coordinate.outer_product(training_dof, training_dof)

    unit_test_spr = ff.PowerSourcePathReceiver(unit_test_frf, 
                                                unit_test_response, 
                                                unit_test_force, 
                                                training_response_coordinate=training_coordinate)
    ...
    assert np.all(reconstructed_training_response.ordinate == unit_test_response.ordinate[:2, :2,:])

    reconstructed_validation_response = unit_test_spr.reconstructed_validation_response
    assert np.all(reconstructed_validation_response.ordinate == unit_test_response.ordinate[2:,2:,:])

    no_force_spr = ff.PowerSourcePathReceiver(unit_test_frf, 
                                               unit_test_response,  
                                               training_response_coordinate=training_coordinate)
...
def test_sample_splitting(unit_test_frf, unit_test_response):
    ...
    assert np.all(unit_test_spr.training_response.ordinate == unit_test_response.ordinate[:2,:2,:])
    assert np.all(unit_test_spr.validation_response.ordinate == unit_test_response.ordinate[2:,2:,:])
    ...
    misordered_training_coordinate = sdpy.coordinate.outer_product(misordered_training_dof, misordered_training_dof)
    misordered_spr = ff.PowerSourcePathReceiver(unit_test_frf, 
                                                unit_test_response,
                                                training_response_coordinate=misordered_training_coordinate)
    ...
    row_index, column_index = np.ix_([0,3], [0,3])
    assert np.all(misordered_spr.training_response.ordinate == unit_test_response.ordinate[row_index,column_index,:])
    row_index, column_index = np.ix_([1,2], [1,2])
    assert np.all(misordered_spr.validation_response.ordinate == unit_test_response.ordinate[row_index,column_index,:])
